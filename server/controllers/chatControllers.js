...
const markAsSpam = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Thêm user vào danh sách đánh dấu spam
    const updatedChat = await Chat.findByIdAndUpdate(
      chatId,
      {
        $addToSet: {
...
const markAsNotSpam = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Xóa user khỏi danh sách đánh dấu spam
    const updatedChat = await Chat.findByIdAndUpdate(
      chatId,
      {
        $pull: {
          spamMarkedBy: { userId: req.user._id },
        },
      },
      { new: true }
    )
    ...
    res.status(200).json({
      success: true,
      message: "Chat marked as not spam",
      chat: updatedChat,
    });
  } catch (error) {
    res.status(400);
...
// Fetch spam chats
const fetchSpamChats = asyncHandler(async (req, res) => {
  try {
